...
**2026-08-14f (AntaresBroker): ETSI-driven coverage added.** `dev/etsi-coverage.sh` (local + CI identical): cargo-llvm-cov-instrumented broker + optional instrumented `cargo test --workspace` (UNIT_TESTS=1) + the dev/etsi-run.sh Robot suites against it, per STORE; merged report → lcov + html + `uncovered-functions.txt` (count-0 = no Rust test AND no ETSI TP ran it). `.github/workflows/etsi-coverage.yml`: weekly Mon 04:41 + dispatch, 4-store matrix + lcov-merge job (0% in merged = untested in every store variation). Report-only, no floor (strict.yml owns the unit floor; ratchet later). Smoke-proven locally: memory + jsonldContext 65/65, SIGTERM flush confirmed (broker-bin files carry counts). Uncovered-fn names are mangled unless rustfilt is installed (html view is demangled regardless).
