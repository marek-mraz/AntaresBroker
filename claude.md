...
5. **Robot Framework TPs — check for existing ones FIRST, then dry-run what you write.** The file's `robot:` list + `grep -r "5_6_6" ngsi-ld-test-suite/TP/` (clause tag form). Only write a NEW TP for normative surface no existing TP covers — duplicating an ETSI TP is noise. New TPs go in the suite fork following its conventions: `[Documentation]` quotes the clause requirement briefly, `[Tags]` carries the clause number (`5_6_6` form) — that tag is what feeds `robot:` back into the ledger. **Edge-case TPs are mandatory**, not optional: error paths, boundary inputs, the cases the official 686 TPs skip. **Every new/changed TP is validated locally before commit** — this respects rule 8 (no broker) while catching broken TPs now instead of one CI round-trip later:
   first `--dryrun` while iterating (catches broken keywords/paths in seconds), then the REAL run per rule 8: the clause's TPs executed against one local broker. `python3 -m json.tool` over each new fixture. The full seven-cell matrix remains CI's job.
...
validation, ledger + claude.md §6 updated. Keep diffs minimal throughout. The goal is
...
**Loop position (2026-08-16, AntaresBroker): matrix (12) — 4 failure rows, ALL 5.13 fleet split-brain residue of b5fa10c, one commit fixes them.** Rows: 053_08_01 hits 3≠2 on BOTH nats cells (deterministic) + timescale-nats 053_06_01 setup 404 and 051_03_01 setup KeyError 'kind' (race-shaped). Cause A (053_06/051_03): after a cross-instance delete, a pod whose FETCHED doc cache was still warm but whose merged cache was cold (any put_local/evict clears merged globally) resolved cold, fetch hit the warm doc → cache_writer never ran → the bump's row-gone signal was DISCARDED (`let _ =`, loader cold path) → row never re-created → next GET 404/no-kind. Fix: cold-path bump that reports row-gone now refetches (bump already evicted the doc) and re-bumps, so a counted use re-creates the entry per 5.13.5.4; merged-hit-path bumps are remembered so the fallthrough rebuild never double-counts surviving sibling URLs. Cause B (053_08): subscription-create `put_local` warms only the creating pod; the OTHER pod resolves the ImplicitlyCreated URL by HTTP through haproxy and serve_context counted that internal fetch as a serve-hit ON TOP of the resolving pod's usage_bump → create contributed 2. Fix: loader fetches carry `x-antares-ctx-fetch: 1` (INTERNAL_FETCH_HEADER) and serve_context skips its bump for marked requests; client serves still count. Red-first both: tests/contexts_fleet_5_13.rs deleted_row_recreated_when_peer_doc_cache_is_warm (reproduced the exact 404) + internal_context_fetch_does_not_count_as_serve_hit (reproduced hits=1 after internal fetch); 3/3 green after, antares-jsonld 63/63, contexts_5_13 5/5, official jsonldContext tree 65/65 local (63 on :9377 + 050_04/051_03 on :9090 — their hardcoded-9090 trap, not failures). ENV TRAP fixed (sandbox-only, not committed): /workspace/.venv site-packages easy-install.pth still pointed at the old /workspace/AntaresBroker path → HttpCtrl import silently gone → every mock-server TP reds with "No keyword 'Start Server'"; repointed to /workspace/ngsi-ld-test-suite/libraries/robotframework-httpctrl/src. Mac-side: push master; next matrix should be 1652/1652 everywhere (053_06/051_03 raced only timescale-nats — watch both nats cells).

**2026-08-14f (AntaresBroker): ETSI-driven coverage added.** `dev/etsi-coverage.sh` (local + CI identical): cargo-llvm-cov-instrumented broker + optional instrumented `cargo test --workspace` (UNIT_TESTS=1) + the dev/etsi-run.sh Robot suites against it, per STORE; merged report → lcov + html + `uncovered-functions.txt` (count-0 = no Rust test AND no ETSI TP ran it). `.github/workflows/etsi-coverage.yml`: weekly Mon 04:41 + dispatch, 4-store matrix + lcov-merge job (0% in merged = untested in every store variation). Report-only, no floor (strict.yml owns the unit floor; ratchet later). TRAP fixed (the big one): cargo-llvm-cov 0.8 instruments via RUSTC_WRAPPER WITHOUT changing cargo fingerprints — correctness depends on `cargo llvm-cov clean`, which silently no-ops (prints "cannot clean", exits 0) when target/ lacks CACHEDIR.TAG; stale UNINSTRUMENTED rlibs then get linked and the report covers a fraction of the workspace (first runs: 4/11 crates, numbers meaningless). Script now creates the tag + hard-fails on a failed clean. Verified after fix: all broker-linked crates in the report incl. antares-sql at ~0% in memory mode (correct); antares-matcher/registry/wasm are NOT linked into the broker binary — ETSI can never cover them, only their unit tests do (UNIT_TESTS=1). TRAP fixed 8596ea9: the script must set ANTARES_SWEEP_SECS=2 (compose-harness parity) or Provision.TransientEntities.422_02 reds on every store; verified Provision 373/373 after. Uncovered-fn names are mangled unless rustfilt is installed (html view is demangled regardless). CORRECTED full-run numbers (post-e3756cb, both runs 1420/1420 robot green): memory+UNIT_TESTS=1 → 83.6% lines (31825 total); file ETSI-only → 70.1% lines (23501; memory>file solely because unit tests merge in — not a store effect); local memory∪file union 85.0% lines (api 93.4 / jsonld 93.3 / ql 96.4 / model 91.6 / sql 48.8 / notifier 59.7 / bus 39.5 / broker 36.2); intersection uncovered-in-both = 1374 fns (results/uncovered-in-both.txt), 880 of them antares-sql pg_* paths that only the CI postgres/timescale cells can cover. antares-matcher + antares-registry are 6/16-line placeholder stubs (zero fns) — their absence from every report is correct, not a bug.

**Loop position (2026-08-14e, AntaresBroker): matrix (9) — 20 failure rows, TWO causes, both fixed.** Matrix (9) ran WITH the 2026-08-14d IOP-campaign commits and caught two campaign regressions: (a) D013_01/02 exc/inc/red on ALL cells (18 rows) = 4698faa's merge_remote_batch tagged every successful upsert batch-forward "created", so an all-updated distributed upsert (source answering 204) returned 201+created-list; fixed 2c6c10b (5.6.8.5: created derives from remote status — 201 lists created ids, 204 = updated; red-first federation_loop clause_5_6_8_remote_204_upsert_is_update_not_create, D013 6/6 local). (b) IOP_EXT_TMP_03_04 pg/timescale only = temporal push_page let SQL pre-apply offset/limit and the 5.7.4.4 fed merge spliced remote rows into the pre-paged set (limit=2 → 3; memory/file never pre-page); fixed ea0f372 (5.7.4.4: push_page requires !would_federate, C11-gate family; red-first PG-GATED tests/temporal_fed_paging_5_7_4.rs vs live PostGIS via private-dind ghcr.io/baosystems/postgis:17-3.5 pgdev recipe — see palace drawer "matrix (9) postmortem"; TMP_03 10/10 run-five; antares-api 32/32 test targets w/ DB env). CLASS LESSON: any store-side narrowing pushdown must gate off when a federated merge follows. TRAP: pg-gated tests on a persistent DB need run-unique fixture ids (type AND registration id) or reruns 409. Also this session: CI #91 spec.py check = unescaped apostrophes in 5.6.8.4/5.7.2.4 notes (f2fd1f9); strict-daily = rustdoc [8]/[digits] (b5fd5f6, pre-existing) + antares-registry unused antares-model dep (ab2ac97); docs/perf-observability-goal.md added (a16c400, unrun). Matrix (11) VERDICT (cells started 09:35:04 UTC = image built between the two fixes): D013 GREEN everywhere (130/130 all cells, file 1652/1652 perfect) — 2c6c10b proven; TMP_03_04 still red on pg/timescale because ea0f372 (09:37:54) was NOT in the image — same 3-row symptom, no new bug, do NOT re-investigate. Mac-side: push master + suite fork, watch the next matrix — with ea0f372 aboard all cells should be 1652/1652.

**Loop position (2026-08-14d, AntaresBroker): IOP TP CAMPAIGN COMPLETE — docs/iop-tp-checklist.md 118/118 [x] with commit+green-run evidence; full IOP_TP tree 220/220 green locally on run-five (102 pre-existing + 118 new).** Twelve groups, PDF-spec-first throughout; every checklist-vs-PDF conflict corrected in place (register/update CSR conflicts are 5.9.2.4/5.9.3.4 not 5.9.4/5.10.4; Via/hostAlias is 6.3.18; entity Notifications have NO triggerReason (5.3.1) and default notificationTrigger excludes deletions (5.2.12); 404 sources are never warnings (6.3.17); client tenant NEVER propagates on forwards (4.14); lang=* returns ONE language (4.15); EntityMap update=5.14.2 ignores output-only members; snapshot partial grades per QUERY (5.16.1.4); federationOps/redirectionOps contain NO temporal ops (4.20)). Broker fixes shipped red-first: 826afac (4.20 query_op gate + 5.2.9 location gate), 43ad459 (6.3.17 peer-warning propagation + single-proxy 508 in combine_attr_parts — the CAS_01_04 flagged red), 4698faa (5.6.7.4/5.6.10.4 batch S/E entity-id arrays via merge_remote_batch, 5.6.8.4 fallbacks drop ?options, 5.6.6.4 ?type gates CSR match, 5.6.1.4 inline @context travels as ld+json), 1ad0fdf (5.8.6 split-merge output shaped/compacted + 5.8.1.4 watchedAttributes narrowed to registered names — the SUB_02 flagged red), 7bb1d3c (5.2.34 management timeout+cooldown HONOURED — the ERR_01_06/07 flagged red — per-reg cooldown in egress.rs distinct from the timeout-only §16.7 breaker; 6.3.17 conn-drop→199; 5.2.9 observationInterval gates temporal fan-out via CsrSpec.temporal; 5.7.3.4 remote tombstones import via allow_null + deletedAt/entity-sysAttrs preserved in sys-expansion). Suite fork: 12 new IOP_EXT files under Interoperability/{Registration,Cascading,ContextSourceInfo,Unitary,Provision/Distributed,Subscription,CSourceSubscription,Temporal,EntityMap,Tenancy,Errors,CrossCutting} + 3 raw-CSR keywords in InteropUtils.resource. Workspace cargo suite green; official HTTP Subscription tree 125/125 (058_x MQTT = mosquitto DNS, CI-only). TRAPS: heredoc/robot after `cd ngsi-ld-test-suite` runs in the fork dir (hit 3×); eval genexps can't see robot-injected locals (use dict-comprehension subscript); HttpCtrl stub-handled requests DO land in the wait queue; a no-stub HttpCtrl mock drops the connection after its 5 s pop-timeout (not a true stall); native io_deadline is a passthrough — per-request reqwest timeout is the real bound. Mac-side: push master (~20 commits) + suite fork (12 TP commits), then watch the 4×8 matrix (pg/timescale exercise the new batch S/E + cooldown + interval-gate paths).

**Loop position (2026-08-14c, AntaresBroker): matrix (8) — ONE failure, root-caused + fixed (commit cd4bc5a `5.8.5.4:`).** Matrix (8): file 1534/1534, timescale 1534/1534, postgres 1533/1534 — the sole red was 5814_01_01 "Wait For Request 15s timeout". Log forensics (decode the zlib string table in log.html; the plain failures.csv keyword is ambiguous — BOTH waits in that TP are `Wait For Request ${15}`): wait #1 PASSED (reduced copy reached the mock, robot replied 201), the loss was the 5.8.5.4 DELETE-forward. Root cause: distsub recorded the remotes mapping only AFTER the create-forward's response, and the httpctrl mock holds its 201 until the test's assertions ran — robot's DELETE then beat the bookkeeping (pg round-trips lose; memory/file in-process maps win, which is why only pg/timescale cells ever flaked: matrix 6 timescale, matrix 8 postgres — same bug, now one cause). Fix: mapping stored BEFORE the forward (remote id is broker-generated), rolled back via targeted mutate on non-2xx (never resurrects a doc a concurrent delete removed). Red-first tests/dist_subs_5_8.rs clause_5_8_5_4_delete_races_slow_create_forward (delayed-reply mock), 5/5 green; TP 5814_01 2/2 local. The matrix-6 "needs pg repro" singleton is RESOLVED — no repro needed, the race is store-agnostic and the unit test pins it. TRAP: the teardown DELETE→404 in the failing log is NORMAL (the in-test delete already 204'd; RequestsLibrary logged no 204 string) — do not read it as "subscription vanished". Mac-side: push master; matrix (6)'s other singleton (timescale 053_07_01 kind=Cached) remains unreproduced/not seen since.

**Loop position (2026-08-14b, AntaresBroker): matrix (7) — ADV_02 root cause CORRECTED.** Matrix (7) (ran WITH 50bcefb+93eaa11): QRY_02_03 + timescale 5814_01_01 gone, but IOP_EXT_ADV_02_01/02/10 still timed out on all cells — the breaker was NOT their cause. Real cause: the CI compose stack (host-networked) sets no ANTARES_PUBLIC_URL, and the default was portless `http://{host_alias}` — broker2's dist-sub notification back to broker1 dialed an unresolvable name (and port 80). run-five passes because it sets the env. Fix (commit 2920e38 `5.8.1.4:`): compose-files/docker-compose-etsi.yml gets ANTARES_PUBLIC_URL http://localhost:909N per broker (the CI unblock) + state.rs default now appends ANTARES_HTTP_PORT when ≠80 (red-first tests/public_url_default_5_8_1.rs; dist_subs_5_8 4/4). 6.3.8 ledger note corrected — do NOT re-attribute ADV_02 to the breaker.

**Loop position (2026-08-14, AntaresBroker): ETSI matrix (6) failures FIXED.** All 13 failure rows map to two causes, both committed: (a) IOP_EXT_ADV_02_01/02/10 (all cells) + 5814_01_01 (timescale) = §16.7 breaker starvation — the suite's one fixed mock host:port ate 5 failures from an earlier test and the 30 s cooldown starved later Wait-For-Request tests; fixed by 50bcefb (6.3.8: breaker trips on timeout-class failures ONLY; responding/refusing-fast destinations record success; U1 stalling guard kept — tests/egress_breaker_6_3_8.rs 2/2 red-first, robot SubscriptionNotificationBehaviour 73/73 HTTP local, IOP Advanced 15/15 on run-five incl. the three ADV_02). (b) IOP_EXT_QRY_02_03 '[]' on pg/timescale = SQL q-pushdown dropped the local half of a split entity pre-merge; fixed by 93eaa11 (5.7.2.4). Neither is pushed — matrix (6) ran on c2eead0. Mac-side: push master (3 commits ahead) + suite fork, then watch the next matrix (pg/timescale cells are where QRY_02_03 proves out; memory brokers can't reproduce it locally). MQTT 058_x fail locally on mosquitto DNS — CI-only per rule 8, not a regression.
