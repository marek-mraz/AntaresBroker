...
4. **Unit tests for the clause's own rules and edge cases — and RUN them.** Boundary values, invalid input → the exact spec error type, empty/absent optional members, multi-instance `datasetId` where applicable, tenant isolation. Test names/doc comments cite the clause number. **Writing tests is not evidence — a green run is**: execute `cargo test -p <crate> -j 2` for the touched crate(s) during the clause work, and every new test must be SEEN to fail if its assertion is inverted or the guarded behaviour is plausible-to-miss (a test that cannot fail proves nothing). Never mark a clause done from tests that were written but not run.
...
7. **One clause = one commit — committed only on a green run**: code, unit tests, Robot TPs, ledger file, message citing the clause number (`5.6.6:` prefix). Before the commit: `cargo test --workspace -j 2` (plus `cargo fmt --all` and clippy) passes — the commit claims "implementation and tests are correct", and only an executed, green workspace run backs that claim.
