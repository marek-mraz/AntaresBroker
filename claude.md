...
suite/spec defects in `/workspace/error.md` + `ngsi-ld-test-suite/testsuite-doubts.md`,
...
6. **Update the ledger file**: `status:` + `evidence:` (code/test anchors) + `notes:` (dates, named gaps, spec doubts); `python3 dev/spec.py robot` refreshes the TP list. Suspected suite/spec defect → prove it from the clause text first, log in `/workspace/error.md` + `testsuite-doubts.md`, never hack the broker to a broken test.
...
- Suspected ETSI-tool bug: prove it from the clause text (MemPalace), log in
  `/workspace/error.md` + `testsuite-doubts.md`, fix the fork if warranted — never hack
...
| Suite/spec defect log | `/workspace/error.md` (workspace-level), `ngsi-ld-test-suite/testsuite-doubts.md` |
...
## 6. Current loop position (state log lives in /workspace/antares-state-handoff-archive.md)

**2026-08-16:** 5.13 fleet split-brain fixed (f3e651d line); wasm-file cell's
last 3 reds (046_01 Content-Length 411) fixed in the Node shim (22cce4e) —
the seam stamps CL for the browser, so the shim now enforces 6.3.4 on the
wire truth. Next full matrix expected 1713/1713 on all seven cells.
Working notes moved to /workspace (error.md, taskImplementation.md,
antares-tasks-archive.md); tasks.md now carries the production-readiness
goal. Mac-side: push master + suite fork; delete stale fed-alias-tenant.
