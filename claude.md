...
**2026-08-14f (AntaresBroker): ETSI-driven coverage added.** `dev/etsi-coverage.sh` (local + CI identical): cargo-llvm-cov-instrumented broker + optional instrumented `cargo test --workspace` (UNIT_TESTS=1) + the dev/etsi-run.sh Robot suites against it, per STORE; merged report → lcov + html + `uncovered-functions.txt` (count-0 = no Rust test AND no ETSI TP ran it). `.github/workflows/etsi-coverage.yml`: weekly Mon 04:41 + dispatch, 4-store matrix + lcov-merge job (0% in merged = untested in every store variation). Report-only, no floor (strict.yml owns the unit floor; ratchet later). Full local runs green: memory UNIT_TESTS=1 = 89.6% lines / 372/373 robot; file ETSI-only = 53.1% (denominator is the whole workspace — pg/timescale crates are 0% by construction in file mode). TRAP fixed 8596ea9: the script must set ANTARES_SWEEP_SECS=2 (compose-harness parity) or Provision.TransientEntities.422_02 reds on every store; verified Provision 373/373 after. Uncovered-fn names are mangled unless rustfilt is installed (html view is demangled regardless).
