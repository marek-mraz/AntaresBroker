...
-- knob wired through the broker's maintenance job (C9/D4).
...
-- both modes: the §8.2 read index + the idempotent-upsert key
...
-- ("columnstore cannot be used on table with row security"). §8.2's memory
...
-- `tenant_id = $1` predicates every store query already carries (§3's
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'timescaledb') THEN
    -- §8.2 compression shape: entity_id in orderby, never segmentby
