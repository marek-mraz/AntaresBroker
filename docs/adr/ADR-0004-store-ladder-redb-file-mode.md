...
Mechanics: one redb table per resource family (spec names, §9.1), key
...
under `block_in_place` so the fsync never stalls a tokio worker (B1b). A
...
file and refuses to start on a format-version mismatch or corruption (B4/B11).
Backup is stop-copy only — redb holds an exclusive file lock and a live copy
can tear mid-commit (B12; tested).
...
per volume); HA needs `postgres` mode (tasks.md §K truth table).
