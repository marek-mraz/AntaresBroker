# ADR-0005: AnyStore enum + synchronous Pg facade (the A1/A3 seam shape)
...
`Result`-returning signatures. Two deliberate deviations from the
deep-analysis §9.3 sketch:
...
2. **Synchronous facade over sqlx.** `PgBackend` runs its async sqlx calls
   ...
   sites in `antares-api` kept their signatures at the C13 cutover — no
   async-ification churn, and the closure-based `mutate` maps 1:1 onto the
   §3.1.2 `SELECT … FOR UPDATE` → apply-in-Rust → version-bump transaction.
...
per-op SQL (UNNEST batches, compiled q= pushdown, C10/C11) lands inside the
