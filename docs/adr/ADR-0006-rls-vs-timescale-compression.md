...
Discovered while landing the C9/D attr_instances decomposition: TimescaleDB
...
refuses RLS/index ALTERs once compression is enabled. §3 (RLS on every
tenant-scoped table) and §8.2 (the 16 GB budget leans on ~90 % temporal
compression) cannot both hold on this one table.
...
first parameter of every store method (§16.1.2). Plain mode keeps the full
RLS belt on attr_instances; every other table keeps RLS in both modes.

Consequences:
- The §16.1.3 "even tenant-less SQL returns zero foreign rows" guarantee has
  one named exception: attr_instances under timescale. The I5 isolation test
