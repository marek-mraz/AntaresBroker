...
- [ ] PRV_01_01 Create forwarded to a redirect CSR supporting createEntity: entity lives on b2 only — assert b1 local=true query does NOT return it (5.6.1.4)
- [ ] PRV_01_02 Create matching a redirect CSR NOT supporting createEntity → 409 Conflict (5.6.1.4, p.160)
- [ ] PRV_01_03 Create matching an inclusive CSR without createEntity support: forward skipped silently, local create 201 (5.6.1.4)
- [ ] PRV_01_04 Forwarded create carries the request @context — remote stores expanded terms identically (5.6.1.4 + 6.3.5)
- [ ] PRV_01_05 Update Attributes split across brokers: local part + redirect part both applied; 204 when all succeed (5.6.2.4 attr-parts)
- [ ] PRV_01_06 Update where the remote part fails → 207 multi-status naming the failed attrs (5.6.2.4)
- [ ] PRV_01_07 Append (noOverwrite) on a remotely-held attr → remote 207/appended; local copy still absent (negative) (5.6.3.4)
- [ ] PRV_01_08 Partial attribute update targeting a redirect-held attr forwards; unknown attr on both sides → 404 (5.6.4.4)
- [ ] PRV_02_01 Delete entity spanning local+redirect: both halves gone; re-retrieve → 404 (5.6.6.4)
- [ ] PRV_02_02 Delete with ?type not matching the redirect-held entity: nothing deleted anywhere (4.17 + 5.6.6.4 negative)
- [ ] PRV_02_03 deleteAttribute of a remote-only attr forwards and returns 204; other attrs untouched (5.6.5.4)
- [ ] PRV_02_04 Batch create with items matching different redirect CSRs: each lands on its broker (5.6.7.4)
- [ ] PRV_02_05 Batch create where one remote item fails → 207 BatchOperationResult with success[] and errors[] correctly split (5.6.7.4, 5.2.16)
- [ ] PRV_02_06 Batch upsert update-mode over a redirect-held entity forwards as update (5.6.8.4)
- [ ] PRV_02_07 Batch delete spanning brokers: 207 lists the remote 404 item, local ones deleted (5.6.10.4)
- [ ] PRV_02_08 Merge entity (PATCH /entities/{id}) with a redirect-held attr forwards the merge fragment (5.6.17.4)

## F. Distributed subscriptions — IOP_EXT_SUB_01/02 (5.8.1.4–5.8.6)

- [ ] SUB_01_01 Subscription update on b1 (new q) re-narrows the remote sub — old-q change no longer notifies (5.8.2.4)
- [ ] SUB_01_02 isActive=false on b1 pauses the chain: remote change produces NO notification; reactivation resumes (5.8.2.4, 5.2.12)
- [ ] SUB_01_03 watchedAttributes reduced copy: remote sub watches only the CSR-scoped attrs — unrelated remote attr change does not notify (5.8.1.4)
- [ ] SUB_01_04 timeInterval (periodic) subscription over federated data: each tick queries remote and notifies the union (5.8.1.4 + 5.2.12)
- [ ] SUB_01_05 Notification data from the remote broker arrives unmodified at the subscriber (values byte-equal) (5.8.6)
- [ ] SUB_01_06 triggerReason=newlyMatching when a remote entity starts matching q (5.8.6, 5.2.14)
- [ ] SUB_01_07 triggerReason=noLongerMatching when the remote update leaves the filter (5.8.6)
- [ ] SUB_01_08 Entity deleted on b2 → deletedAt-style notification through the chain (5.8.6, 4.5.7)
- [ ] SUB_02_01 splitEntities=true: one merged notification entity assembled from b2+b3 parts (5.8.6 merge block; deployment default off — enable per-sub)
- [ ] SUB_02_02 splitEntities merge honours q on the AGGREGATE (5.8.6 + 5.2.23 splitEntities row)
- [ ] SUB_02_03 Sub created BEFORE the CSR: registering the CSR later wires the chain (registration-triggered sub propagation, 5.8.1.4)
- [ ] SUB_02_04 CSR deleted → remote leg dismantled; remote change stops notifying while local changes still do (5.8.5.4 negative pair)
- [ ] SUB_02_05 Two subs sharing one CSR: deleting one keeps the other's chain alive (5.8.5.4 refcount edge)
- [ ] SUB_02_06 Remote notification whose subscriptionId is unknown (stale/forged) → dropped, no client delivery (5.8.6 inbound gate + security negative)
