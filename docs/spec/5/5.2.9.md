...
evidence: ''
notes: 'partial signals 2026-08-10 (full audit pending): the tenant member selects the peer tenant on every forward (wasm handle.rs csr_tenant_federates_across_tenants_in_one_broker); contextSourceAlias is read into FedReg.alias and excludes visited sources from matching (federation.rs matching_regs, test registered_alias_in_the_via_chain_is_not_a_matching_registration).'
robot: []
---
