...
pages: 111-115
status: implemented
evidence: 'csource.rs normalize_registration: id URI (generated when absent at create), type pinned to ContextSourceRegistration, endpoint URI mandatory, information non-empty with 5.2.10/5.2.8 element validation + cardinality caps, contextSourceInfo KeyValuePair[] with the 4.3.6.6 constrained keys, observation/managementInterval TimeIntervals, mode value space, expiresAt future DateTime, tenant per the 4.14 rules; ADDED THIS AUDIT: operations limited to the 4.20 names+groups (federation.rs ALL_OPERATION_NAMES/OPERATION_GROUPS), localOnly boolean, contextSourceAlias non-empty RFC 7230 pseudonym token, description/registrationName non-empty, datasetId URI[]/@none, scope per the shared 4.18 grammar (antares-jsonld valid_scope_value), location/observationSpace/operationSpace as 4.7 geometries, refreshRate ISO 8601 duration. Behavioural members: tenant on forwards (4.14 audit), alias loop-exclusion + localOnly forwarding (4.3.6.4/ADR-0011), operations matching (4.20). Unit lib::clause_5_2_9 (17 value-space cases, red 2026-08-11 before the arms). TPs: Registration dirs 35/35 + 037_11 3/3 green vs local memory broker 2026-08-11.'
notes: '2026-08-14b: observationInterval/managementInterval now gate temporal fan-out (CsrSpec.temporal + matching_regs overlap check via temporal_interval_matches; interval-less registrations stay unconstrained — both members are optional). TP IOP_EXT_TMP_03_07 red-first on run-five. audited 2026-08-11: the value-space half of the table was unenforced (tolerant reader swallowed operations/localOnly/alias/refreshRate/datasetId/scope/geometries/name-strings) - all added. management member (5.2.34) and RegistrationManagementInfo defaults audit at their own clauses.
  2026-08-14 IOP campaign fix: the location member never gated consumption fan-out — CsrSpec.geo now
  carries the query geo filter and matching_regs skips registrations whose location geometry does not
  match it (GeoQuery::matches_geometry, same matcher as 5.10.2.4 discovery; registrations without
  location stay unconstrained). TP IOP_EXT_REG_01_14 red-first on run-five, green after.'
robot: []
---
