...
notes: 'audited 2026-08-12 (Snapshot API built — the optional group is now OFFERED). Orthogonal to Tenant:
  both NGSILD-Snapshot and NGSILD-Tenant must be given, cross-tenant ids 404 (asserted). The MAY resource-pressure
  eviction (priority-ordered) is not implemented — snapshot metadata is per-process memory, so a restart
  drops snapshots, which the clause explicitly allows; promote to the store + sweep orphaned snap- tenants
  if durable snapshots are required.'
robot: []
---
