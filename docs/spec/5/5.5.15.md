...
notes: 'audited 2026-08-12 (Snapshot API built — the optional group is now OFFERED). Orthogonal to Tenant:
  ...
  resource-pressure eviction IS implemented (cap knob). Snapshot metadata is per-process memory, so a
  restart drops snapshots, which the clause explicitly allows; promote to the store + sweep orphaned snap-
  tenants if durable snapshots are required.'
robot: []
---
