...
evidence: 'crates/antares-api/src/snapshots.rs snapshot_layer + synthetic per-snapshot tenant: every Core/Temporal
  ...
  red->green) clause_5_5_15_snapshot_is_tenant_scoped + freeze assertions in clause_5_16_1_* (deferred
  item 3): resource-pressure eviction implemented — snapshots.rs evict_over_cap (per-tenant AppState.snapshot_cap,
  default 1024): lowest snapshotPriority first, earliest expiresAt breaks ties, the just-created snapshot
  is never the victim; evicted snapshots with an endpoint get the 5.3.4 deletion notification (expiresAt
  < notifiedAt). Test clause_5_5_15_priority_eviction_over_cap (red seen — unit-only: a TP would need
  a non-default snapshot_cap broker).'
