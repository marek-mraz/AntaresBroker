...
pages: '121'
status: implemented
evidence: 'subscriptions.rs notification/endpoint validation: uri mandatory valid URI with a scheme gate (http/https + mqtt/mqtts under the feature; unsupported scheme -> 422 OperationNotSupported), accept limited to json/ld+json/geo+json defaulting to application/json at delivery, cooldown/timeout numbers > 0, receiverInfo/notifierInfo validated as KeyValuePair[] (added this audit), MQTT notifierInfo params parsed at creation (7.2). Delivery-side use: receiverInfo headers + accept on the notifier path (notify.rs), cooldown honoured after failures. Unit lib::clause_5_2_15 (7 HTTP cases; red 2026-08-11 on the KeyValuePair shape). Subscription TPs 33/33 (5.2.14.1 run).'
