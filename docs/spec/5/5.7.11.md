...
pages: 217-218
status: implemented
evidence: 'Posture clause, all MAY: Antares takes the sanctioned first option — discovery (5.7.5-5.7.10)
  answers from the local datastore only (types_attrs.rs type_stats/attr_stats over local entities). No
  forwarding fan-out; counts are exact for local data.'
notes: If registry-derived possibly-available types are ever wanted, the clause permits empty attributeNames/attributeTypes
  arrays and count estimates — revisit only if asked for (the forwarding alternative is explicitly more
  expensive).
robot: []
---
