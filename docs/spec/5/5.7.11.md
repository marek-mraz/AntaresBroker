...
pages: 217-218
status: implemented
evidence: 'Posture clause, all MAY: Antares takes the sanctioned first option — discovery (5.7.5-5.7.10) answers from the local datastore only (types_attrs.rs type_stats/attr_stats over local entities). No forwarding fan-out; counts are exact for local data.'
notes: 'audited 2026-08-12. If registry-derived possibly-available types are ever wanted, the clause permits empty attributeNames/attributeTypes arrays and count estimates — revisit only on user request (the forwarding alternative is explicitly more expensive).'
robot: []
---

5.7.11
Architecture-related aspects of retrieval of Entity Types and
Attributes
...
As an alternative to relying on local information only, the request can be forwarded to all Context Sources which
support the respective operation according to the Context Source Registration describing them. In this case
the returned lists are merged with the local list of entity types before returning them. This approach is more expensive
but leads to a more accurate result.
