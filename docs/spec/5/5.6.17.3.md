---
clause: 5.6.17.3
title: Input data
pages: '187'
status: implemented
evidence: 'entities.rs merge_entity: id URI, ?type selector (added this audit), observedAt/lang/format/options params, Entity Fragment via expand_entity (allow_null + merge — the 5.5.4/5.5.12 null semantics).'
notes: 'audited 2026-08-11.'
robot: []
---

5.6.17.3
Input data
•
A URI representing the id of the Entity to be merged (target Entity).
•
A selector of Entity types as specified by clause 4.17 (optional).
•
A JSON-LD document representing an NGSI-LD Entity Fragment.
•
An optional flag indicating whether the JSON-LD document contains a simplified representation of the entity.
•
An optional parameter indicating a common observedAt timestamp to use across merged Attributes.
•
An optional parameter representing a common IETF RFC 5646 [28] language tag to use across merged
LanguageMap Attributes.
