...
pages: 362-364
status: implemented
evidence: 'antares-notifier/src/mqtt.rs: MqttEndpoint::parse (mqtt[s]:// URI with user:pass@host:port/topic/subtopics, default ports 1883/8883), MqttParams (Table 7.2-1: MQTT-QoS 0|1|2, MQTT-Version mqtt3.1.1|mqtt5.0, defaults QoS 0 + mqtt5.0, invalid values 400 at subscription time), build_message ({metadata, body} with Content-Type + Link-in-metadata for json, @context-in-body for ld+json — Table 7.2-2 tests); receiverInfo entries are added first so a pair keyed like a Table 7.2-2 parameter stays an entry and never becomes that parameter''s value, whose source the table fixes (receiver_info_cannot_take_over_a_table_7_2_2_parameter); delivery via notify.rs deliver() MQTT arm (bounded pooled sink). Notifier lib tests 5/5 green'
notes: 'audited 2026-08-12: MQTT TPs (docker mosquitto) stay CI-only per the standing rule-8 harness; the 4x8 CI matrix runs them per push. mqtts TLS via rumqttc.'
robot: []
---
