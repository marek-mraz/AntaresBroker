...
pages: 362-364
status: implemented
evidence: 'antares-notifier/src/mqtt.rs: MqttEndpoint::parse (mqtt[s]:// URI with user:pass@host:port/topic/subtopics, default ports 1883/8883), MqttParams (Table 7.2-1: MQTT-QoS 0|1|2, MQTT-Version mqtt3.1.1|mqtt5.0, defaults QoS 0 + mqtt5.0, invalid values 400 at subscription time), build_message ({metadata, body} with Content-Type + Link-in-metadata for json, @context-in-body for ld+json — Table 7.2-2 tests); delivery via notify.rs deliver() MQTT arm (bounded pooled sink). Notifier lib tests 5/5 green'
