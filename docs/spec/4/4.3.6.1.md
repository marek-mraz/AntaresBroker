...
evidence: 'crates/antares-api/src/federation.rs: FedReg::supports() (operations subset gates every forward,
  ...
  TP 436_01 (negative operations gate + attrs narrowing) green locally 2026-08-10. 2026-08-13: fan_out()
  — forwards to matching registrations run CONCURRENTLY (bounds::MAX_FED_FANOUT, env ANTARES_FED_FANOUT
  default 8) across fed_query/fed_retrieve/fed_query_temporal/fed_retrieve_temporal/fed_entity_maps;
  results processed in registration order so 4.5.5 merge + 6.3.17 warning order stay deterministic.
  Test fed_fanout_4_3_6.rs (3 half-second sources answered in ~0.5 s, merge completeness + no-duplicate
  asserted; red-first at 1.52 s sequential); DistributedOperations/Consumption tree 43/43 local.
  2026-08-13 FIX (found by IOP_EXT_ADV_02_08 vs two real brokers): batch distribution ignored the
  registration''s EntityInfo constraints — a type-scoped read-only redirect reg conflicted EVERY batch
  item and the out-of-scope item was never created locally. FedReg::covers_item (ids/types gate; a
  type-less fragment stays covered) now gates reduce_to_scope + strip_proxied. Red-first lib test
  batch_items_outside_a_registrations_types_stay_local.'
notes: 'Audited 2026-08-10. Broker-binding SHALLs: (1) "all constraints specified in the registration
  shall be respected" on forwarded requests — implemented for reads and writes; (2) "Context Brokers shall
  respect" a limited operations subset — implemented in supports(). The remaining SHALLs bind registered
  Context Sources, not the forwarding broker: normalized representation as default and tolerance of requested-but-absent
  ...
  on forwards is a "can", not a SHALL. 2026-08-15: forwarded-id-list narrowing enforced in fed_query ("all constraints specified in the registration shall be respected" incl. Entity IDs) — FedReg::can_match_id; red-first tests/id_routing_5_12.rs clause_4_3_6_1_forwarded_id_list_narrowed_to_registration + over-narrowing guard (type-only EntityInfo keeps the full list). TP IOP_EXT_IDR_02_11. Batch writes gate items the same way since 2026-08-15: covers_item + batch-delete sent_ids route through can_match_id (idPattern-scoped CSRs receive only their ids; found red by IOP_EXT_IDR_03_24/28/29).'
robot:
- '436_01'
---
