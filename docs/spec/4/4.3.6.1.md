...
evidence: 'crates/antares-api/src/federation.rs: FedReg::supports() (operations subset gates every forward,
  ...
  strip_proxied. Red-first lib test batch_items_outside_a_registrations_types_stay_local. merge_same_source
  folds registrations of one source into one forward and unions their entity/attribute scope, so it folds
  only registrations whose EXPANDED operation subset is the same (op_set over OPERATION_NAMES, since 5.2.9
  lets the member name groups): otherwise an operation one registration declared would travel for the
  other''s Entities, which is the subset this clause says shall be respected. Red-first lib test
  registrations_of_one_source_that_offer_different_operations_do_not_fold.'
