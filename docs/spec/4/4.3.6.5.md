...
evidence: 'crates/antares-api/src/federation.rs: FedReg.csi carries the registration''s contextSourceInfo
  ...
  ignored) green locally 2026-08-10.'
notes: Audited 2026-08-10. "Tenant shall not be part of contextSourceInfo" is enforced by ignoring the
  pair on the wire (the clause's own remedy for binding-specific pairs — "if present, such information
  shall be ignored" — applied uniformly); registration-time rejection was not added since the clause prescribes
  ignoring, not erroring. The 4.3.6.6 processed keys (accept, contentType, jsonldContext, ngsildConformance)
  are audited under 4.3.6.6.
robot:
- '436_05'
---
