...
pages: 77-78
status: implemented
evidence: 'entities.rs parse_join() (join=inline|flat|@none, joinLevel 1..=MAX_JOIN_LEVEL bounded
  traversal), lookup_joined()/collect_flat() retrieve only locally stored targets;
  duplicate suppression via the flat BTreeMap key and joined-entity dedup; joinLevel caps
  cascade length. Official 018_1x/019_1x/046_2x TPs green (30 cases run locally
  2026-08-10).'
notes: 'Audited 2026-08-10. objectType-annotated non-local targets are a distributed-retrieval concern (5.7.x/CI).'
robot: []
---
