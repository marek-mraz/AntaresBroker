...
pages: '96'
status: implemented
evidence: 'antares-api/src/entities.rs: paginate_impl returns the total only when count=true (relayed "whenever this is requested by the client" -> NGSILD-Results-Count on every list endpoint: entities, subscriptions, csource, temporal, batch query) + page_params allows limit=0 exactly when count=true (the clause''s zero-results-with-count case) and rejects it otherwise. Unit: entities::clause_4_13 (2 tests). TP 019_31_05 (limit=0&count=true -> empty page, count 3, no links); the header mechanics are 6.3.13, audited there.'
notes: 'audited 2026-08-11. Counting is independent of limiting (tested both ways); no gaps.'
robot:
- '019_31'
---
