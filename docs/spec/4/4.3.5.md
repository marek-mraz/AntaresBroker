...
pages: 35-40
status: partial
evidence: 'crates/antares-api/src/lib.rs router() (doc comment cites 4.3.5) registers
  every functionality group of Table 4.3.5-1: Core (entities, entityOperations,
  subscriptions), Temporal (temporal/entities, local per Table 4.3.5-2), Registry
  (csourceRegistrations, csourceSubscriptions — local/integrated), discovery
  (types, attributes), JSONLDContext (jsonldContexts), distributed support
  (info/sourceIdentity = 5.15.1).'
notes: 'Audited 2026-08-10. Umbrella clause; its SHALLs ("all Context Brokers shall
  implement the Core API"; distributed brokers "shall also implement the Distributed
  API") delegate wholesale to the §5 operation clauses, each audited under its own
  file. GAP keeping this partial: EntityMap support operations 5.14.1–5.14.3 have
  no /entityMaps routes (entityMap exists only as a query parameter) — a Distributed
  API support surface the table mandates for distributed deployments; close when
  the 5.14.x clauses are implemented. Snapshot API (5.16) absent but explicitly
  optional. Configuration: integrated temporal + integrated Context Registry.'
robot: []
---

4.3.5
NGSI-LD API Structure and Implementation Options
...
Operations
Core API
Context Information Provision - operations for
...
Entity (distributed)
5.6.13 Delete Attributes from Temporal
Evolution
...
Registry API
Context Source Registration - operations for
