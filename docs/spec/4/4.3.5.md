...
pages: 35-40
status: implemented
evidence: Core + Distributed API operation clauses all recorded under their own files; /entityMaps + /temporal/entityMaps
  routes exist since the 5.14 build (entity_maps.rs); Snapshot API offered since (snapshots.rs; Table
  4.3.5-2, staged-v1x)
notes: 'reconciled after the 5.14 EntityMaps build — the named gap (no 5.14.1-5.14.3 routes) is closed.
  Configuration: integrated temporal + integrated Context Registry.'
robot: []
---
