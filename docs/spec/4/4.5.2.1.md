...
evidence: 'crates/antares-api/src/repr.rs: both representations supported and consumer-selectable — options=concise|normalized
  and the format parameter parse into Repr (normalized is the default; concise applies the 4.5.2.3 transforms
  on render).'
notes: 'Umbrella introduction — its one SHALL ("both normalized and concise representation of Properties
  shall be supported … selected through specific request parameters") delegates: member-level rules recorded
  under 4.5.2.2 (normalized) and 4.5.2.3 (concise); the request-parameter surface under 6.3.7.'
robot: []
---
