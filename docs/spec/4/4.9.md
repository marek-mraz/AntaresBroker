...
pages: 84-92
status: partial
evidence: 'antares-ql/src/lib.rs (full ABNF: QPath = linked-entity hops attr{[T[,T]:]…} + dotted path
  ...
  -> None -> in-memory post-filter); unit tests: antares-ql 13, antares-api qeval tests+clause_4_9_extensions
  12 (red seen on kind_allows inversion 2026-08-11); TP 019_28 + 019_29 19/19 green vs local memory broker
  2026-08-11'
notes: '2026-08-12: gap (1) CLOSED — notify.rs conditions_match resolves linked-entity q hops through
  the store (tests/notify_linked_q_4_9.rs, TP 49_01); gap (2) CLOSED — expandValues wired on the temporal
  query (5.7.4.3; linked hops on temporal stay 400 by 5.7.4.4 design, so the temporal null lookup is unreachable).
  REMAINING (keeps this partial): UCA collation / RFC 3986 par.6 URI minimization SHOULDs — codepoint
  comparison only (4.23.x build). Snapshot member filtering is staged-v1x posture, not a code gap.'
robot:
- 019_28
- 019_29
- '49_01'
