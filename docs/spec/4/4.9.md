...
pages: 84-92
status: partial
evidence: 'antares-ql/src/lib.rs (full ABNF: QPath = linked-entity hops attr{[T[,T]:]…} + dotted path + single trailing [member.path]/[lang]/[*] bracket; ValueList/Range/URI value rules pinned to ==/!=, notPatternOp, and/or precedence) + antares-api/src/qeval.rs (eval_q/resolve_qpath/apply_expand_values: Equal/Unequal/array/languageMap/Range/ValueList semantics, VocabProperty target expansion, Relationship+ordering-op never matches, EXAMPLE 9-14) + antares-sql/src/compile/q.rs (links/bracket outside the exact SQL subset -> None -> in-memory post-filter); unit tests: antares-ql 13, antares-api qeval tests+clause_4_9_extensions 12 (red seen on kind_allows inversion 2026-08-11); TP 019_28 + 019_29 19/19 green vs local memory broker 2026-08-11'
notes: 'audited 2026-08-11. expandValues + jsonKeys accepted on 6.4.3.2 entity query (jsonKeys is a deliberate no-op: raw JSON targets are never term-expanded to begin with). GAPS: (1) subscription/notification q terms with linked-entity hops evaluate as non-matching - the matcher has no store access (notify.rs conditions_match passes a null lookup); wire or reject at the 5.8.x audit. (2) temporal q evaluates with a null lookup and no expandValues wiring - whether 6.18+ accepts those params is that clause''s table; decide there. (3) Snapshot member filtering (Table 5.2.43-1) - Snapshots are staged-v1x. (4) UCA collation and RFC 3986 par.6 URI-comparison minimization are "should"s - plain codepoint comparison per RFC 8259 8.3 only. CSource filtering by Context Source Properties is 5.10/6.8 surface, audited there (q attr-roots already feed registration matching per 5.10.2.4).'
robot:
- 019_28
- 019_29
---
