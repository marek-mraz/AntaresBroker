...
pages: 84-92
status: implemented
evidence: 'antares-ql/src/lib.rs (full ABNF: quotedStr = RFC 8259 String, escapes decoded and re-escaped on render, while a patternOp/notPatternOp operand stays the verbatim RegExp the grammar names; an unquoted value carrying a bare quote is refused (neither URI nor dateTime/date/time admits one); leading space before a value trimmed before the quoted/unquoted decision, so the spacing of the clause example `color!= "black", "red"` keeps the quotes out of the value; QPath = linked-entity hops attr{[T[,T]:]…} + dotted path
  + single trailing [member.path]/[lang]/[*] bracket; ValueList/Range/URI value rules pinned to ==/!=,
  notPatternOp, and/or precedence) + antares-ql/src/eval.rs (ordering operators and both inclusive Range bounds compare 4.6.3 DateTime and Time on a canonical key, so p.92 "the order relation considered shall be a temporal one" holds across the optional seconds fraction; a Date needs none and an ordinary string stays a string) + antares-api/src/qeval.rs (eval_q/resolve_qpath/apply_expand_values:
  Equal/Unequal/array/languageMap/Range/ValueList semantics, VocabProperty target expansion, Relationship+ordering-op
  never matches, EXAMPLE 9-14) + antares-sql/src/compile/q.rs (links/bracket outside the exact SQL subset
  -> None -> in-memory post-filter); unit tests: antares-ql 85 (RFC 8259 quotedStr: escaped quote/backslash/\uXXXX decode, undefined escape refused, regexp operand verbatim, render round trip), antares-api qeval tests+clause_4_9_extensions
  12 (red seen on kind_allows inversion 2026-08-11); TP 019_28 + 019_29 19/19 green vs local memory broker
  2026-08-11'
notes: '2026-08-12: linked-entity q in notifications CLOSED (notify.rs store lookup, TP 49_01); expandValues
  on temporal CLOSED (5.7.4.3). Remaining SHOULDs deliberately declined and documented: q string comparison
  stays codepoint order (UCA ordering available via the 4.23.3 collation parameter on orderBy; adopting
  UCA inside q operators would change match semantics broker-wide for a SHOULD) and RFC 3986 par.6 URI
  minimization is not performed. Snapshot member filtering is staged-v1x posture.'
robot:
- 019_28
- 019_29
- '49_01'
