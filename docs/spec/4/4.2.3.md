...
pages: 30-31
status: implemented
evidence: 'the cross-domain concepts exist as first-class typed attributes: GeoProperty/LanguageProperty/scope/unitCode/observedAt
  in antares-jsonld/src/expand.rs (validated at expansion); TP 408_01 covers the temporal value space'
notes: 'umbrella clause — every SHALL delegates (geo->4.7, temporal->4.8, unitCode->4.5.2, scope->4.18,
  LanguageMaps->4.5.18), each recorded under its own clause. NO ontology/RDF machinery belongs in the
  broker (user decision): the RDF/RDFS grounding is representational and carried by JSON-LD expansion,
  same posture as 4.2.2.'
robot: []
---
