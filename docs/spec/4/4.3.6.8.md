...
evidence: 'crates/antares-api/src/conformance.rs implements Tables 4.3.6.8-1/2/3: amend_entity/amend_attr
  ...
  held ("any JSON value" per Table 4.3.6.8-2, String or String[] per Table 4.3.6.8-3), so they carry through
  untouched instead of taking Notes 2/3. Extension TP 436_08 (Prefer 1.5 reformat + native no-op) green
  locally.'
notes: '"Context Sources should indicate the version the returned payload actually conforms to" — Antares
  answers Preference-Applied as that indication; the consumer side tolerates peers that do not send it
  (the received payload is validated and amended locally regardless).'
robot:
- '436_08'
- '436_09'
---
