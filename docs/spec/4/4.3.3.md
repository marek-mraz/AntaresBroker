...
status: informative
evidence: ''
notes: 'Audited 2026-08-10. Prose description of Figure 4.3.3-1 (Context Sources
  hold the data, register capabilities with the Context Registry; the Distribution
  Broker discovers, forwards, aggregates). Zero SHALLs — the normative distribution
  behaviour lives in 4.3.6.x and the §5 distributed-operation clauses and is audited
  there. The D0xx robot list below is tag-derived: those TPs exercise the concrete
  clauses, kept here as cross-reference only. Rule-9 verdict: architecture umbrella,
  zero code.'
robot:
- D001_01_exc
- D001_01_inc
