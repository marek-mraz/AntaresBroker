...
evidence: 'crates/antares-api/src/federation.rs: inclusive default (mode unwrap_or "inclusive" in reg
  ...
  locally 2026-08-10.'
notes: 'Audited 2026-08-10. "Auxiliary always accepted as there can never be a conflict" is satisfied
  by construction: CSR creation applies no mode-conflict validation to auxiliary (the 4.3.6.3 exclusive-overlap
  validation gap tracked in claude.md §6 does not involve aux). Consumption-only enforcement is structural
  (write_regs) rather than per-op checks. Equal-priority inclusive fan-out is the concurrent forward in
  fed_read/fed_write paths.'
robot:
- '436_02'
- D001_01_inc
- D001_02_inc
