---
clause: 4.5.13
title: Attribute List Representation
pages: '66'
status: implemented
evidence: 'antares-api/src/types_attrs.rs attributes() (GET /attributes, no details): id is a
  generated urn:ngsi-ld:AttributeList URN, fixed type "AttributeList", attributeList of
  compacted attribute names. Integration test: discovery::attribute_representations_shapes
  (incl. no-extra-members negative check; proven fallible). Official TP 025_01 green against
  the local memory broker 2026-08-10.'
notes: 'Audited 2026-08-10. No gap.'
robot: []
---

4.5.13
Attribute List Representation
The attribute list representation is used to consume information about attributes. The attribute list representation shall be
a JSON-LD object containing the following members:
Mandatory
•
"attributeList": JSON-LD array containing the attribute names.
•
"id": whose value shall be a URI that identifies the attribute list.
•
"type": the fixed value "AttributeList".
Optional
•
"@context": a JSON-LD @context as described in clause 4.4.
