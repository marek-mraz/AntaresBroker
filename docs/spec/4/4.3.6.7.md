...
evidence: 'The unitary-operation behaviour itself is implemented: distributed queries/retrieves fan out
  ...
  allowed-params).'
notes: query-side closed at the 5.14 build; retrieve-side + temporal wiring closed (entities.rs retrieve_entity_outer,
  temporal.rs retrieve_temporal_outer/query_temporal_outer, federation.rs map_gate). Configurability tail
  (entityMapLifetime knob + broker ceiling) satisfied by entity_maps.rs expires_at.
robot: []
---
