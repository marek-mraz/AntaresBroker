...
evidence: 'The unitary-operation behaviour itself is implemented: distributed queries/retrieves fan out
  ...
  allowed-params).'
notes: '2026-08-12: reconciled after the 5.14 EntityMaps build. Query-side surface CLOSED: EntityMap created/cached
  (bounded in-memory store), NGSILD-EntityMap header returned (Table 6.4.3.2-2), a supplied map is reused
  with filter-recheck + stale-entry pruning during pagination, 5.14.1-5.14.5 CRUD routed, splitEntities
  filter-stripping on create. REMAINING (keeps this partial): the RETRIEVE-side wiring — entityMap/header
  on GET /entities/{id} and on the temporal retrieve/query — is accepted but not yet honoured (no retrieve_entity_outer
  / query_temporal_outer); wire like entities.rs query_entities_outer.'
robot: []
---
