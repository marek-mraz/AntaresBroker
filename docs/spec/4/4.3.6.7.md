...
pages: 43-44
status: partial
evidence: 'The unitary-operation behaviour itself is implemented: distributed
  queries/retrieves fan out one broad request per matching registration and
  aggregate before rendering (crates/antares-api/src/federation.rs fed read paths,
  merge_candidates; exclusive registrations are consulted directly without any map,
  as the clause infers). The entityMap query parameter is accepted on
  /entities and /temporal/entities (entities.rs/temporal.rs allowed-params).'
notes: 'Audited 2026-08-10. GAP (named, keeps this partial): the EntityMap
  mechanism is not implemented — no EntityMap resource is created or cached, the
  NGSILD-EntityMap response header (Table 6.4.3.2-2) is never returned, a supplied
  entityMap is ignored rather than reused, the 5.14.1–5.14.5 CRUD operations have
  no routes (same gap named in 4.3.6.5''s sibling 4.3.5), and split-entities
  candidate handling with re-filtering during pagination ("any Entity not fulfilling
  the filter shall be removed from the EntityMap") does not exist. The subsystem''s
  normative surface is defined by 5.14.x + 6.3.19/6.4.3.2-2 — implement there, then
  upgrade this clause and 4.3.5. The clause''s "should enable configuring a default"
  for the split-entities parameter joins that work.'
robot: []
---
