...
evidence: 'expand.rs GeoProperty arm: string value parsed iff valid JSON + validate_geojson, normalized
  to the object form; the iff is the whole of "the syntax and restrictions mandated by IETF RFC 7946
  [8] when representing a valid Geometry of the type specified" - check_geometry walks the coordinates
  against the declared type and rejects a non-number coordinate (4.7.2 "values of a JSON-LD floating
  point number data type") and a position outside the WGS84 range; antares-ql geo.rs parse_geometry
  calls the same validator, so a reference geometry the broker would not store is a 400 and a stored
  target that is not a geometry is a non-match. Tests geo_property_rules + geojson_geometries_meet_the_rfc_7946_restrictions
...
notes: '2026-08-10: string encoding accepted per the iff rule and NORMALIZED to the geometry object on
  output (one representation for storage, geo-queries, responses); coordinates stay JSON floats (serde),
  precision = f64.'
robot:
- '471_01'
---
