...
pages: 99-101
status: implemented
evidence: 'antares-api/src/federation.rs: FEDERATION_OPS / ASSOCIATION_OPS / REDIRECTION_OPS / UPDATE_OPS / RETRIEVE_OPS verbatim from Tables 4.20-1/2 + FedReg::supports (individual name or group; default when absent = federationOps per the clause''s closing sentence, applied at registration compile). Unit federation::clause_4_20 (association excludes the four EntityMap ops — red seen 2026-08-11 before the fix; group membership spot checks incl. createEntityMapQueryTemporal being in NO group). Dist-ops regression TPs 436_01 + 436_04 5/5 green vs local memory broker 2026-08-11.'
notes: 'audited 2026-08-11. Fixed during audit: associationOps was aliased to federationOps, wrongly forwarding the four EntityMap support operations to sources that never offered them (4.3.6.1 "shall respect"). The enforcement SHALL itself lives in 4.3.6.1 (audited); this clause is the name/group tables. Temporal/batch/EntityMap operation names are matched by string today; their forwarding paths arrive with the corresponding 5.x clauses.'
robot: []
---
