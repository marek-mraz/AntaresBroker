...
evidence: 'Isolation: ADR-0001 shared schema, TenantId as first param of every store method; tests/tenant_isolation.rs
  ...
  mock-asserted) 3/3 + 061_01 + 436_05 regression green vs local memory broker. jsonld_contexts was the
  one Tenant-bearing table outside the belt: ADR-0021 and migration 0006_context_tenant give it a GENERATED
  tenant_id and an RLS policy, so a stored @context — term mappings 5.5.7 makes decide what a Tenant''s
  payloads mean — is isolated by the database and not by a called function alone (pg_rls_pentest.rs
  every_tenant_bearing_table_carries_the_belt_or_a_written_exemption now leaves only tenants exempt).'
