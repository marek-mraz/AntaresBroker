...
pages: '96'
status: implemented
evidence: 'Isolation: ADR-0001 shared schema, TenantId as first param of every store method; tests/tenant_isolation.rs (cross-tenant probe indistinguishable from nonexistence for entities/subscriptions/registrations, tenant-keyed mirrors, tenant-keyed store state). Default tenant: tenant_from() -> "default" when the header is absent (every tenant-blind test exercises it). CSR tenant on forwards: federation.rs forward() sends NGSILD-Tenant only when the REGISTRATION carries a tenant member - fixed this audit (was flowing the requesting tenant through); tests/federation_topology.rs tenantless_registration_targets_the_peer_default_tenant (red seen 2026-08-11 before the fix) + cross-tenant chain/cycle tests for the tenant-carrying CSR path. Tenant-specific Via alias = ADR-0011 (4.3.6.4 audit). TP 414_01 (isolation + header echo + tenant-less-CSR forward carries no header, mock-asserted) 3/3 + 061_01 + 436_05 regression green vs local memory broker 2026-08-11.'
notes: 'audited 2026-08-11. FIXED: a tenant-less CSR forwarded the REQUESTING tenant to the peer - 4.14 mandates "no Tenant information is to be used and thus the default Tenant is targeted" (also a tenant-name leak). NoMultiTenantSupport is N/A (multi-tenancy supported); NonexistentTenant/implicit tenant creation are 5.5.10/6.3.14 surface, audited there. POSTURE: the tenant value space is charset-restricted to token-safe strings (deep-analysis - the tenant is embedded in the Via pseudonym per Table 5.2.40-1, and RFC 7230 tokens exclude ":"), so URN-shaped tenants get 400 BadRequestData; the spec only says "represented in form of a String" and leaves tenant creation implementation-specific.'
robot:
- '061_01'
- '414_01'
---
