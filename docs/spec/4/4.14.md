...
notes: 'audited 2026-08-11. FIXED: a tenant-less CSR forwarded the REQUESTING tenant to the peer - 4.14 mandates "no Tenant information is to be used and thus the default Tenant is targeted" (also a tenant-name leak). NoMultiTenantSupport is N/A (multi-tenancy supported); NonexistentTenant/implicit tenant creation are 5.5.10/6.3.14 surface, audited there. POSTURE: the tenant value space is charset-restricted to token-safe strings (the tenant is embedded in the Via pseudonym per Table 5.2.40-1, and RFC 7230 tokens exclude ":"), so URN-shaped tenants get 400 BadRequestData; the spec only says "represented in form of a String" and leaves tenant creation implementation-specific.'
robot:
- '061_01'
- '414_01'
---
