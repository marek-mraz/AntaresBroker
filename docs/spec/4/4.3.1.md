...
status: informative
evidence: ''
notes: 'Architecture-umbrella introduction. Its one SHALL ("the NGSI-LD API shall enable efficient support
  for all of them") binds the design of the API itself — a constraint on ETSI, not on implementations;
  no broker-observable behaviour. The Core-vs-Temporal split is definitional; the "should consider updating
  the Temporal Evolution when current state is modified" recommendation is realized by the temporal mirroring
  recorded under the 5.6.x write operations and the Temporal API clauses. Rule-9 verdict: delegation note,
  zero code.'
robot: []
---
