...
pages: 72-73
status: implemented
evidence: 'temporal.rs parse_iso_duration() (ISO 8601 durations incl. P[n]W, PT0S/P0D =
  whole-range period), aggr_eligible() (Table 4.5.19.1-1/2/3 per-datatype eligibility;
  ineligible → InvalidRequest per 5.7.4.4), classify_instance()/numeric_of()
  (string lexicographic min/max, array-size aggregation, boolean-as-number, Time-of-day avg),
  aggregate_bucket() (avg/distinctCount/max/min/stddev/sum/sumsq/totalCount; V-27
  no-infinity guard). Same tests/TPs as 4.5.19.0.'
