...
status: informative
evidence: ''
notes: 'Definitional introduction: compacted representation + the NGSI-LD Null encodings (urn:ngsi-ld:null;
  single-element array; {"@none": "urn:ngsi-ld:null"} for languageMap; JSON null literal via @json). Zero
  SHALLs of its own — the operational rules are recorded where they bind: deletion-by-null in the 5.6.x
  update/merge/patch clauses (e.g. fragment member removal per 5.4), languageMap null in 4.5.18, deleted-element
  representation in notifications (5.8.6) and temporal (5.6.x temporal ops). Rule-9 verdict: concept clause,
